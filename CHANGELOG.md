...
- Browser: bind the final GPT-5.6 Sol + Pro proof, exact composer/controller, hit-tested send target, and trusted primary click into one fail-closed dispatch guard. The current collapsed composer may expose only a bare `Pro` pill, so Oracle now treats that as mode-only evidence and reads the exact current Sol version from composer-owned React state; it also refuses an open picker when Sol is merely available but not active. Model/mode drift, React remounts, overlays, disabled controls, untrusted events, and indeterminate post-press failures can no longer be reported as a clean pre-submit refusal or recovered through a DOM-click fallback.
