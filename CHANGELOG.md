...
- Browser/Remote: recognize Chromium's flattened Linux process title when proving an externally managed Chrome's profile, DevTools port, PID, kernel-reported executable, and process generation, while rejecting duplicate ownership-critical flags as ambiguous. This prevents healthy attach-only workers from reporting `attach-target-owner-mismatch` after a worker-only restart without trusting a forged process title as Chrome identity.
...
- Browser: bind the final GPT-5.6 Sol + Pro proof, exact composer/controller, hit-tested send target, and trusted primary click into one fail-closed dispatch guard. The current collapsed composer exposes only a bare `Pro` pill, so Oracle now treats it as mode-only evidence, opens the exact composer's causally owned public menus, and requires unique checked `Pro` and `GPT-5.6 Sol` rows with agreeing selection markers. A short-lived, mutation-sensitive proof is then consumed at the trusted button-capture click; unchecked availability triggers, duplicate exact rows, lookalike-only evidence, model/mode drift, remounts, overlays, disabled controls, untrusted events, and indeterminate post-press failures are refused without relying on private React state or a DOM-click fallback.
...
- Remote: log the accepted browser worker's run, account, and lane immediately and persist account/lane attribution in successful session evidence, so concurrent fleet failures can be tied to the actual VM and lane instead of being misdiagnosed as shared-browser contention.
- Browser/Remote: treat ChatGPT's transient `/c/WEB:<client-id>` route as provisional across submission, response capture, reattach, cookies, and follow-ups; adopt the first canonical conversation ID monotonically; derive a bounded discovery preview by normalizing the complete submitted prompt before truncation, then bind submission, capture, and recovery to a SHA-256 of the full rendered user-turn DOM instead of a lossy prefix, with both canonicalization algorithms explicitly versioned in signed recovery claims; preserve backward-compatible Markdown/DOM discovery across composer confirmation, attachment binding, sidebar navigation, prompt-echo rejection, and reattach; bind copied Markdown and Deep Research reports to the exact submitted message; require every ordinary remote `done.ok` result to carry a fresh message-handle binding plus challenge-clean proof; and preserve worker-reported terminal post-submit failures as account-pinned, capture-only recovery sessions with origin-isolated, DOM-bound v2 private capabilities and single-writer completion. Remote failures now advertise only account-affine recovery, exit nonzero when recovery fails before commit, and persist typed retryability so `oracle restart` permits only proven pre-submit browser failures. Recovery capabilities are minted only in the worker's terminal failure event after an exact committed-turn binding: a transport cut, client death, old v1 record, or unbound follow-up leaves route evidence but no executable recovery coordinate, so the originating ChatGPT account's history remains the fallback. Incomplete same-run multi-turn sessions likewise refuse single-answer capture-only completion until per-turn results can be durably checkpointed.
- Browser/Remote: keep protected ChatGPT dispatch, capture, and recovery on one exact composer/controller/submitted-turn identity across DOM remounts and asynchronous model proof; refuse ambiguous protected sends instead of retrying them, reject assistant metadata copied from a different snapshot, recheck access state after artifact finalization, durably quarantine only independently corroborated account access walls, and preserve the exact headful challenge tab and lease for human-only clearance while ordinary failures still close and release their browser target.
- Fable: require an explicit CAAM subscription profile for the reviewed `fable-local` lane (while preserving the compatibility-only direct `--engine claude-code` route), keep Fable at fixed `xhigh` effort, clamp automatic CAAM rotations to zero even when a positive value is inherited, verify only the exact model identities at startup, persist each reviewed one-shot under the exact builder-owned Claude session UUID, use Claude's real `--resume` primitive for same-profile/base follow-ups, refuse older non-persistent sessions instead of starting an unrelated conversation, recover complete answers from verified plan/no-tools protocol episodes, and fail closed with raw artifacts when that terminal protocol is malformed or ambiguous instead of publishing it as an answer.
- Browser/Remote: update the protected ChatGPT lane to the current two-axis `GPT-5.6 Sol` + checked `Pro` UI, require both selections before every prompt (including resumed runs), recover once from a stale initial picker with a hard refresh before failing closed, carry model/mode proof through remote terminal events into session metadata, and keep visible or status-combined `Answer now` placeholders classified as generation liveness without ever clicking them or discarding real explanatory prose.
...
- CLI: enable reviewed `--lane chatgpt-pro` and `--lane gemini-deep-think` aliases for the existing browser-backed Pro and Gemini Deep Think routes, preserving legacy engine/model commands while making `oracle --help` show one parallel lane example set.
...
- Config: default clean installs and configs without explicit overrides to browser automation with GPT-5.5 Pro Extended reasoning.

### Fixed

- Browser: close the remaining thinking-pause truncation holes that returned one-line teasers (~42 tokens) for ChatGPT Pro/Pro-Extended answers: thinking activity now resets the idle clock in both acceptance loops (a >60s pause no longer collapses acceptance to a one-sample finished-controls flicker), non-compact acceptance in thinking territory must hold across consecutive samples, bare compact acceptance is blocked once a wait outlasts the compact grace window (measured from wait start, including through the recovery path), the post-race guard confirms mid-thinking and preamble-sized captures via the watchdog (with a bounded calm-page budget, stability-checked last-resort read, and loud failure instead of silent teasers), snapshot refresh can no longer shrink a good capture, and the two divergent thinking-indicator detectors were unified into one shared, localization-aware predicate with structural streaming markers.
- Remote: `oracle serve` logs answer chars/tokens on run completion so confidently-wrong short captures are forensically visible server-side.
...
- Browser: report `response streaming` from the thinking-status heartbeat when a visible stop control is the only remaining liveness signal, and share that signal with completion capture so ChatGPT selector drift cannot finalize a still-streaming answer. Fixes #284. Thanks @StartupBros!
- Browser: make remote-CDP ChatGPT runs recoverable when background windows or watchdogs interfere by enabling focus emulation and persisting the `/c/<id>` conversation URL shortly after submit.
- Browser: re-poll one-character ChatGPT captures and skip automatic archiving for suspiciously short answers, preserving the conversation for harvest/recovery instead of deleting a possibly still-streaming response.
