...
- Fable: keep `fable-local` at fixed `xhigh` effort and clamp automatic CAAM rotations to zero even when a positive value is inherited, verify only the exact Fable model identities at startup, and preserve the selected CAAM profile/base across follow-ups.
