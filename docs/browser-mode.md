...
- `--browser-queue-timeout`: independent FIFO wait for a shared browser tab slot (default `20m`). It also accepts `browser.queueTimeoutMs` or `ORACLE_BROWSER_QUEUE_TIMEOUT`; invalid environment values fall back to the default. Local `0` means unlimited waiting, while remote requests drop `0` and clamp positive values to `1s`–`75m`.
- `--browser-max-concurrent-tabs`: soft limit for simultaneous ChatGPT tabs sharing one manual-login profile (default `3`). Set `ORACLE_BROWSER_MAX_CONCURRENT_TABS` for a per-host default; explicit CLI/config values win. Additional runs wait up to the independent queue timeout for a slot and log `[browser] Waiting for ChatGPT browser slot...`.
...
- The reviewed ChatGPT lane verifies two independent UI axes in the same live Intelligence menu: the exact visible model label `GPT-5.6 Sol` and a separately checked bare `Pro` mode. If the initial picker snapshot lacks the requested model, Oracle hard-refreshes the new-chat page once and repeats the check; it then **fails closed before prompt submission** if either axis remains missing or ambiguous. Resumed conversations are never refreshed by this recovery. The legacy `gpt-5.5-pro` and generic Pro aliases map to this current target for compatibility. Detection failures write a bounded, redacted model-picker diagnostic to the normal session log.
...
- `oracle import-chatgpt-url <url> --slug "<words>"`: register a manually started ChatGPT thread as an untrusted URL reference. The import records no answer, model/mode proof, lane, or account. Continue it only with `oracle --engine browser --remote-browser off --browser-model-strategy current --followup <slug> -p "..."`; reviewed lanes, remote/account-affine routing, harvest, and recovery are refused.
- `oracle import-chatgpt-url <url> --slug "<words>" --force`: replace only a pure prior import. Replacement requires descriptor-rooted filesystem operations and is unavailable on Windows; choose a new slug there.
...
- `--model`: the same GPT-5.6 aliases work in API and browser mode. Use `gpt-5.6` for the current GPT-5.6 default or `gpt-5.6-sol` to pin Sol; browser mode maps either alias to the `GPT-5.6 Sol` picker entry, while API mode sends the corresponding first-party OpenAI model ID. The current reviewed ChatGPT target is `gpt-5.6-sol` plus `--browser-thinking-time extended`; prefer `--lane chatgpt-pro`, which injects and protects both settings and verifies the independent Pro mode. GPT-5.5, GPT-5.4, and GPT-5.2 variants remain compatibility paths, and legacy Pro aliases resolve to the latest GPT-5.6 Sol + Pro target.
...
A persisted FIFO waiter queue prevents later processes from jumping ahead when capacity opens. `--browser-queue-timeout <duration>`, `browser.queueTimeoutMs`, or `ORACLE_BROWSER_QUEUE_TIMEOUT` controls only this admission wait, independently of the response and profile-lock deadlines. Expiry for a new run is a typed, retryable, pre-submit `browser_lock_timeout`/`capacity_busy` failure and removes the waiter atomically. The same timeout during recovery or reattach remains non-retryable because the original run has already submitted.
...
- A `[retry] Requested model was absent from the initial picker snapshot` log means ChatGPT served a stale picker shape. Oracle performs one pre-submit hard refresh and re-verifies the exact model and mode; a subsequent `model-selection` error means the account still does not expose the requested model and no prompt was sent.
