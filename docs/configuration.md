...
```json5
{
  ...
  browser: {
    ...
    queueTimeoutMs: 1200000, // independent FIFO wait for a shared browser slot (default: 20m; local 0 = unlimited)
...
- Remote browser execution currently resolves `ORACLE_REMOTE_HOST` / `ORACLE_REMOTE_TOKEN` first, then `--remote-host/--remote-token`, then `browser.remoteHost` / `browser.remoteToken` in the config. Diagnostic attach is the exception: `oracle remote attach --host ... --token-env ...` treats the typed host as the explicit target. Clear stale `ORACLE_REMOTE_HOST` / `ORACLE_REMOTE_TOKEN` values before relying on a router cutover stored in config.
...
- `--followup <sessionId|responseId>` continues a saved ChatGPT browser conversation, a Fable/Claude Code session under the exact same CAAM profile/base, or an OpenAI/Azure Responses API run. Browser followup reopens the exact conversation and inherits the parent's browser profile, configuration, and model; API followup accepts a stored Oracle session id or a `resp_...` Responses API id.
