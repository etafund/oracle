...
This is the whole first-try surface — the 3 reviewed lanes, plus the shared session commands every lane uses:
...
Everything below this point is compatibility/advanced surface — still fully supported and runnable, just not the first-try path. The CLI itself hides these same commands/flags from default `--help`; run `oracle --help --verbose` to list them from your installed build.

<details>
<summary>Compatibility &amp; advanced usage (OpenAI/Azure follow-ups, dry-run/perf-trace, project sources, image generation, bridge/remote diagnostics, TUI)</summary>
...
# Register a manually started ChatGPT thread for an untrusted compatibility follow-up
npx -y @steipete/oracle import-chatgpt-url "https://chatgpt.com/c/abc123" --slug "manual browser review"
npx -y @steipete/oracle --engine browser --remote-browser off \
  --browser-model-strategy current --followup manual-browser-review -p "Continue this thread"
...
- Manual ChatGPT URLs can be registered with `oracle import-chatgpt-url`, but the reference is explicitly untrusted, account/lane-unbound, and answer-free. It can continue only through an explicit local `--engine browser --remote-browser off` compatibility route using the current conversation model; it is never eligible for a reviewed lane or harvest/recovery.
- Import replacement with `import-chatgpt-url --force` is narrower still: it replaces only a pure prior import and requires descriptor-rooted filesystem operations. It is unavailable on Windows, where you must choose a new slug.
...
Visible in default `--help` — the flags the 3 reviewed lanes and everyday session management need:
...
| `--browser-queue-timeout`                                                      | Independent FIFO wait for a shared browser tab slot (default 20m; local `0` waits forever).                                                                                                                                                                       |
...
| `--models <list>`                                                     | Compatibility-only comma-separated API fan-out across built-ins and provider/OpenRouter IDs. Not part of the reviewed lane surface.                                                                                                             |
