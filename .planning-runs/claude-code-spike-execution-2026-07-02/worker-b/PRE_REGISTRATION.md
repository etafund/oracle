...
- Unsupported active requests return `agent_lane_blocked`, exit code `2`, replacement commands for all reviewed lanes, and `noBackendStarted: true`.
...
Success criteria:
- `fable` / `claude_code_fable_5` are scoped only to `claude-code` / `fable-local`.
...
- JSON and human stderr outputs are enough for a calling agent to retry once without reading docs.
- Prompt text and secrets are not present in block output.

Kill criteria:
- Errors only say "see help" or provide placeholders that are not executable templates.
