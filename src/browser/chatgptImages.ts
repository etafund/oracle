...
export async function saveChatGptGeneratedImages(params: {
  ...
  for (let index = 0; index < trustedImages.length; index += 1) {
    ...
    try {
      ...
      try {
        const { response, finalUrl: downloadedUrl } = await downloadChatGptImageWithCookies({
          url,
          cookieHeader,
        });
        if (!response.ok) {
        ...
        logger?.(
          `[browser] ChatGPT generated image download failed via Node fetch; retrying in browser context (${image.fileId ?? url.toString()}: ${message}).`,
        );
        const browserFetch = await fetchGeneratedImageInBrowserContext(Runtime, url.toString());
